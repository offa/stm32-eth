...
namespace eth
{
    ...
    std::uint16_t Socket::send(const gsl::span<const std::uint8_t> buffer)
    {
        ...
        const std::uint16_t sendSize = std::min<std::uint16_t>(m_device.getTransmitBufferSize(), buffer.size());
    ...
    std::uint16_t Socket::receive(gsl::span<std::uint8_t> buffer)
    {
        ...
        const std::uint16_t sizeLimited = std::min<std::uint16_t>(m_device.getReceiveBufferSize(), buffer.size());
